  ...
  try {
    const content = await llmChat(
      [
        {
          role: "system",
          content: "Classify the user intent. Reply with a single token: INFO, BATTLE, COUNTERS, TEAM, or REPORT.",
        },
    ...
    if (process.env.DEBUG) console.log("classifyWithLLM fallback:", err);
    return "INFO";
  }
}
